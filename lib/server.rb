...
class Server
  ...
  def request
    ...
    while true
      ...
      headers = ["HTTP/1.1 302 Found", "Location: http://127.0.0.1:9297/game\r\n\r\n"].join("\r\n")
      @ps.info = client.read(response.content_length(request))
      print_to_client(client, request, headers, output)
  ...
  def print_to_client(client, request, headers, output)
